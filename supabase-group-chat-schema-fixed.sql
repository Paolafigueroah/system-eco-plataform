-- Schema corregido para chat grupal
-- Ejecutar en Supabase SQL Editor

-- Primero, verificar la estructura actual de la tabla conversations
-- Si no tiene las columnas necesarias, agregarlas:

-- Agregar columna para tipo de conversación si no existe
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'conversations' AND column_name = 'conversation_type') THEN
        ALTER TABLE conversations ADD COLUMN conversation_type VARCHAR(20) DEFAULT 'private' CHECK (conversation_type IN ('private', 'group'));
    END IF;
END $$;

-- Agregar columnas para grupos si no existen
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'conversations' AND column_name = 'group_description') THEN
        ALTER TABLE conversations ADD COLUMN group_description TEXT;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'conversations' AND column_name = 'group_image_url') THEN
        ALTER TABLE conversations ADD COLUMN group_image_url TEXT;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'conversations' AND column_name = 'group_admin_id') THEN
        ALTER TABLE conversations ADD COLUMN group_admin_id UUID REFERENCES profiles(id);
    END IF;
END $$;

-- Crear tabla para participantes de grupos si no existe
...
DROP POLICY IF EXISTS "Group participants are viewable by group members" ON group_participants;
DROP POLICY IF EXISTS "Admins can manage group participants" ON group_participants;
...
-- Función corregida para obtener conversaciones del usuario
...
BEGIN
    ...
    SELECT 
        ...
        CASE 
            WHEN c.conversation_type = 'private' THEN
                CASE 
                    WHEN c.buyer_id = user_id_param THEN c.seller_id
                    ELSE c.buyer_id
                END
            ELSE NULL
        ...
        CASE 
            WHEN c.conversation_type = 'private' THEN
                CASE 
                    WHEN c.buyer_id = user_id_param THEN 
                        COALESCE(seller_profile.display_name, seller_profile.email, 'Usuario')
                    ELSE 
                        COALESCE(buyer_profile.display_name, buyer_profile.email, 'Usuario')
                END
            ELSE c.group_name
        ...
        CASE 
            WHEN c.conversation_type = 'private' THEN
                CASE 
                    WHEN c.buyer_id = user_id_param THEN seller_profile.email
    ...
    LEFT JOIN profiles buyer_profile ON c.buyer_id = buyer_profile.id
    LEFT JOIN profiles seller_profile ON c.seller_id = seller_profile.id
    WHERE 
        (c.conversation_type = 'private' AND (c.buyer_id = user_id_param OR c.seller_id = user_id_param))
        OR
...
-- Trigger para actualizar contadores automáticamente (solo si la tabla review_helpful existe)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'review_helpful') THEN
